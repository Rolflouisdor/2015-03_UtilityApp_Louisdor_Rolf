...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk2015_03_UtilityApp_Louisdor_RolfUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk2015_03_UtilityApp_Louisdor_RolfUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k2015_03_UtilityApp_Louisdor_Rolf app using the node.js CLI
