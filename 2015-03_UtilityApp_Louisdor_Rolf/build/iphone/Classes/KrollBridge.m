...
NSString * k2015_03_UtilityApp_Louisdor_Rolf$ModuleRequireFormat = @"(function(exports){"
...
@implementation k2015_03_UtilityApp_Louisdor_RolfObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k2015_03_utilityapp_louisdor_rolf);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k2015_03_utilityapp_louisdor_rolf gc];
}
...
{
	// create k2015_03_UtilityApp_Louisdor_Rolf global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k2015_03_UtilityApp_Louisdor_Rolf" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k2015_03_utilityapp_louisdor_rolf = [[k2015_03_UtilityApp_Louisdor_RolfObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k2015_03_utilityapp_louisdor_rolf];
	
	NSString *_k2015_03_utilityapp_louisdor_rolfNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k2015_03_utilityapp_louisdor_rolfNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k2015_03_utilityapp_louisdor_rolf valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k2015_03_UtilityApp_Louisdor_Rolf$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k2015_03_utilityapp_louisdor_rolf.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.k2015_03_utilityapp_louisdor_rolf.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
